...
Three Lighthouse fixes landed 14 Sep 2026, each with a maintenance cost that a future edit can silently break. Read this before touching `assets/js/main.js`, any of the four CSS files, or the `services-pin__poster` images.

**Critical CSS.** Every page's `<head>` inlines a hand-extracted `<style>` block — this project's permitted exception to "no inline `<style>` blocks" — covering tokens.css's `:root` custom properties (no `@font-face`; those aren't needed for a correct first paint under `font-display: swap`, so they're left out to keep the block smaller and avoid a relative-URL bug: `@font-face`'s `url()` paths are relative to `tokens.css`'s own location, not to whichever page inlines a copy of them), base.css in full, and a curated header/nav/button/eyebrow/page-intro subset of components.css. `index.html` additionally inlines the `.hero__*` rules, the only page that needs them. The real four files still load too, non-blocking: a `<link rel="preload" as="style">` per file starts the fetch early, then `assets/js/critical-css.js` — a classic, non-deferred `<script src>`, not `main.js` — creates the real `<link rel="stylesheet">` elements in JS (never as static HTML tags) so a `load` listener is always attached before the fetch can possibly finish; a static `media="print"` link race-loses for small, fast files (confirmed by hand — tokens.css and base.css raced ahead of a later script's listener attachment while components.css, slower, did not), which is why this isn't the simpler static-link-plus-media-swap version of the pattern. A `<noscript>` fallback loads all four normally with JS disabled. **If you change a rule inside tokens.css's `:root` block, base.css, or the header/nav/button/eyebrow/page-intro/hero portions of components.css, update the matching inline `<style>` block on every page by hand** — there is no build step to keep the two copies in sync automatically, and Lighthouse and the live page will disagree with each other until you do.

**Deferred module init.** `main.js` no longer initialises every module synchronously on load. `lenis-scroll`, `header-pill`, `contact-form`, `hero-headline` and `hero-tilt` still run immediately — the only things above the fold on any page. Everything else (`services-pin` onward, in the same relative order as before, testimonial modules still last) waits for the first scroll event or a `requestIdleCallback` (2000ms timeout, `setTimeout` fallback for Safari), whichever fires first. The pin-spacer ordering rule this file has always followed still holds: `services-pin` must still run before every other `ScrollTrigger`-creating module in the deferred list, for the same reason `hero-headline` had to run before `services-pin` in the old single synchronous block. This is a scheduling change only — no module's internals changed, there is still exactly one RAF loop (`gsap.ticker`), and `ScrollTrigger` is still registered once.

**Right-sized posters.** The four `services-pin__poster` images ship at 744×418 (2× their displayed 372×209, for high-DPR screens), re-encoded from the 1280×720 source frames with `ffmpeg`/libwebp at quality 82. The full 1280×720 frames stay as-is for the `<video>` elements themselves — only the poster stills shrink. If the displayed size in `components.css` (`.services-pin__panel`'s `aspect-ratio`) ever changes, re-encode the posters to match and update the `<img>` width/height attributes alongside it, or CLS regresses.
