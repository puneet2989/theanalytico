...
| # | Item | Files | Action |
|---|---|---|---|
| 1.1 | **Draft testimonials awaiting sign-off** — real clients, unconfirmed wording: Raj (ArdLens), Kat (KC Accountants), Piotr (SodoLT) | `index.html` testimonial slides | These are now attributed to real, named clients, but the wording is drafted, not their actual words. Get each person's written approval of their quote (or their own wording) before this goes live indexable — publishing invented text under a real person's name is a bigger risk than a fabricated placeholder name, and in the EU/Ireland a false consumer endorsement is prohibited under the Unfair Commercial Practices Directive as transposed by the Consumer Protection Act 2007. |
| 1.2 | **Invented metrics** — "+42% organic sessions", "1.2s LCP" | `index.html` KPI block | Replace with measured figures from real client sites, or delete the block. No `AggregateRating` was ever emitted in JSON-LD, so the structured data stays truthful either way. |
...
| 1.6 | **AI-generated background video** — abstract terracotta/periwinkle ribbon footage, generated via Seedance from a text prompt (not filmed, not client footage; no people, logos or recognisable objects, so no consent/licensing subject). Confirm the generation service's output license permits commercial use before launch. | `work.html`, `#showcase` `<video data-showcase-hero-video>`, `assets/video/promo-bg.mp4`/`promo-bg-poster.webp` (same file also used on the unpublished `promo.html` review page) | Needs licence confirmation before launch. |
| 1.7 | **Invented metric** — "98 average Lighthouse performance score" | `work.html` "The countable part" KPI block | Same placeholder figure as `index.html` row 1.2. Replace with a real measured score from the three live sites, or delete the KPI. |
...
| 3.9 | Maintenance and care plan scope, pricing and response times | `[EVIDENCE NEEDED: maintenance plan scope, pricing and response times]` | `services.html`, `#web-design` maintenance add-on | CLAUDE.md confirms maintenance is part of Web Design (decided 12 Sep 2026) but states no scope, pricing or SLA — none is invented here. Confirm with the client what a plan actually includes (update frequency, what counts as a "small change," response time) and how it's priced (flat fee, tiered, hourly). Rewrite the maintenance paragraph to state it directly once confirmed, and remove the marker comment above it in `services.html`. |
...
| 4.4 | KC Accountants scroll-capture video was recorded from the interim `kc-accountant.puneetcf.workers.dev` URL, same as row 4.1 | `assets/img/work/kc-accountants-capture.webm`, `.mp4`, `-poster.webp` | Recapture once the real production domain exists — a scroll-through of the interim URL is fine to ship now (the URL itself is not visible anywhere in the capture's on-page presentation), but re-record so the footage matches whatever the domain ends up serving, in case the real deployment differs from the workers.dev preview. `[EVIDENCE NEEDED: KC Accountants production domain]` |
...
| 5.4 | KPI disclaimer line: `Preview figures. Real measurements replace these before launch.` | `docs/specs/02-home.md` §9 | `index.html`, `#results` | Delete this line once 5.1 and 5.2 are resolved. The fourth KPI (`98` average Lighthouse performance score) was already dropped rather than shipping a self-reported score below the project's own 95+ bar — see `docs/REVIEW-2026-09-12.md` P0-3. |
| 5.5 | Three testimonials, real names now, wording still draft: `Raj, ArdLens`, `Kat, KC Accountants`, `Piotr, SodoLT` | `docs/specs/02-home.md` §10 | `index.html`, `#testimonials`, three carousel slides | Get each client's written approval of their quote (or their own wording) before publishing — see row 1.1. Photos were dropped rather than reused stock images presented as their portraits; only add a photo if the client supplies their own. |
